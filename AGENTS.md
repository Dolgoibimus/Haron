...
**Текущая версия:** 0.21.0 (Phase 2, Batch 21)
...
Batch 21 завершён. Room DB + глобальный поиск + индексация + WorkManager + ContentObserver + ScreenOnReceiver.
Видео превью в проводнике: overlay-подход реализован, но проблема чёрного экрана НЕ решена (см. Известные проблемы).
...
### Чёрный экран при воспроизведении видео/аудио в превью (QuickPreviewDialog)

**Симптом:** При тапе Play в превью (как для видео, так и для аудио) — аудиодорожка играет, но вместо видео/обложки — чёрный экран. При тапе на паузу overlay уходит и экран возвращается к нормальному виду (thumbnail).

**Что пробовали:**
1. Lifecycle observer (ON_START/ON_STOP) → двойной `attachViews`, краш `IllegalStateException`
2. `activeVlcLayout` трекер → старая страница отвязывала поверхность новой
3. `autoStartPage: Int` вместо boolean → флаг сбрасывался до чтения пейджером
4. **Overlay-подход** (текущий): VLCVideoLayout вынесен из пейджера наружу как постоянный overlay (по аналогии с MediaPlayerScreen) — аудио играет, но видео всё равно чёрный экран

**Корневая причина:** VLC `attachViews()` внутри `AlertDialog` (отдельная субкомпозиция, `DialogWrapper` + своя `ComposeView`) — VLC не может нормально рендерить на поверхность внутри диалога. В MediaPlayerScreen (полноэкранный, не диалог) VLC работает идеально.

**Возможные направления:**
- Заменить `AlertDialog` на полноэкранный `Dialog(properties = DialogProperties(usePlatformDefaultWidth = false))` — даст свой Window
- Полностью отказаться от VLC в превью — показывать только thumbnail + кнопку «Открыть в полноэкранном плеере»
- Использовать ExoPlayer/Media3 вместо VLC для превью (но это второй движок)
...
### Читалка документов
- Чтение DOC, DOCX, ODT, RTF файлов прямо в приложении
...
### Защищённая папка (Shield)
- Шифрование файлов AES-256-GCM (ключ в Android Keystore, потоковое без загрузки в память)
- Кнопка Shield в панели действий: защитить или снять защиту с выделенных файлов/папок
- Защита рекурсивная: все файлы внутри папки шифруются, папка удаляется
- Снятие защиты: файлы и папки восстанавливаются на оригинальное место
- Иконка замка на защищённых файлах в списке и сетке
- Тап на защищённый файл → расшифровка в кэш → открытие во встроенном просмотрщике
- Долгий тап на щит в шапке панели → ввод PIN/биометрия → виртуальная папка «Все защищённые файлы»
- Тап на щит (подсвечен) → выход из режима защиты
- В виртуальном режиме: копирование, перемещение, удаление, превью, «Открыть в...» — всё работает
- Создание папок и текстовых файлов в защищённых директориях
- Пункт «Защищённые файлы» в боковом меню (счётчик и размер)

### Блокировка приложения
- PIN-код (4–8 цифр) и/или биометрия на вход
- 4 режима: отключено, PIN, биометрия, биометрия + PIN
- Автоблокировка при уходе в фон (grace period 3 сек)
- Точки PIN соответствуют реальной длине PIN
- Автопроверка при наборе полного PIN (без кнопки OK)
- Shake-анимация + вибрация при неверном PIN
- Режим «Биометрия + PIN»: сначала отпечаток, при отмене — PIN-pad
- Защита от скриншотов (FLAG_SECURE) когда заблокировано
- Настройки: выбор метода блокировки, установка/смена PIN
...
### Защищённая папка
- Шифрование файлов AES-256 (ключ в Android Keystore — невозможно извлечь)
...
- Room DB + FTS5 для быстрого поиска
