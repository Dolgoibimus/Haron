...
| Площадка | Статус | Папка / ссылка |
|----------|--------|----------------|
| 4PDA | Конкурентный анализ проведён, публикация планируется | `I:\Saebalissimus\Документы\Android Studio\Haron\4PDA\` |
| RuStore | v1.6 опубликован | — |
| GitHub | v1.6 опубликован | `Dolgoibimus/Haron` (private) |
| Haron TV | В разработке (shared + tv модули, D-pad, leanback) | `C:\Users\User\AndroidStudioProjects\HaronTV` |
...
### Batch 103 — Сессия 2026-03-24: навбар развитие + фиксы ✅ проверено
...
### Batch 102 — Сессия 2026-03-23: APK размер, библиотека, читалка, SD-карта, навбар ✅ проверено
...
### Batch 90 — Поиск: keyword-типы + multi-type + LibGen + Web Navigator ⏳ в ожидании
...
### Batch 89 — Поиск: SearXNG + умные дорки + contentType в движках ⏳ в ожидании
...
### Batch 88 — Поиск: confirmation card + прогресс-бар облака ⏳ в ожидании
...
| S3 протокол | Amazon-совместимые облака (Backblaze B2, MinIO, Wasabi). Растущий рынок self-hosted хранилищ. MiXplorer пользователи просят годами |
| MEGA облако | 20ГБ бесплатно, популярное. API нестабилен — у всех конкурентов слетает каждые 3 месяца |
| Mail.ru Облако | Российское облако. Не поддерживает НИКТО нормально (MiXplorer только через WebDAV, медленно). Пустая ниша РФ |
| Сбер Диск | Российское облако. Не поддерживает ВООБЩЕ НИКТО. Полностью пустая ниша |
| DLNA браузер | Просмотр файлов на DLNA-устройствах (NAS, ТВ). У нас DLNA каст есть, но не браузер — логичное дополнение |
| FTP как Document Provider | Haron регистрируется как SAF Document Provider для FTP/SMB — любое приложение видит сетевые диски через системный пикер |
| Встроенный ADB shell (LADB-подход) | Доступ к Android/data без отдельного Shizuku APK. Бандлим ADB-бинарник, пользователь один раз вводит pairing code из Wireless Debugging → Haron получает shell UID 2000. После ребута на A14+ нужно повторять. LADB уже в Google Play — подход допустим |
| TMDB информация о видео | Подтягивание названия, описания, постера фильма по имени файла. Есть у X-plore |
| Редактирование ZIP на месте | Добавить/удалить файл из ZIP без полной перепаковки. Есть у X-plore, ZArchiver |
...
| Автозадачи | Правила "когда X → сделай Y": триггер по событию (новый файл в папке → переместить по расширению) или по расписанию (каждый день → синхронизация). Экспорт/импорт правил между устройствами. Объединяет наблюдатель за папкой + запланированные операции |
...
#### TODO — Генератор стилей кнопок навбара

**Концепция:** встроенный конструктор стилизованных кнопок по текстовому запросу (без AI, словарь стилей).

**Словарь стилей (ключевые слова → параметры Canvas):**
- "огненная" → градиент красный→оранжевый→жёлтый + частицы пламени
- "морозная" → голубой→белый + кристаллики
- "неоновая" → glow + яркая обводка
- "стеклянная" → полупрозрачный + блик
- "круглая/квадратная/скруглённая" → форма
- 20-30 слов в словаре

**UI и логика:**
1. Генерирует сразу **все кнопки** которые есть в списке для навбара
2. Результат сохраняется как **тема навбара** (набор стилей для всех кнопок)
3. В NavbarSettingsScreen — сворачиваемая кнопка выбора темы (список сохранённых тем)
4. Кнопка "Повтор" на каждой кнопке отдельно (перегенерация одной) + общий повтор всего набора
5. При генерации для навбара у которого уже есть набор — новые кнопки **добавляются** (до 9 шт), не заменяют

**Хранение:**
- Темы: JSON в SharedPreferences (имя темы → список стилей кнопок)
- Каждый стиль: цвета, градиент, форма, эффекты, иконка

---

## Архитектура — кто кого вызывает

#### TODO — Кнопка обновления сети (после VPN/смены Wi-Fi)

**Проблема:** после отключения VPN Android кеширует маршруты и DNS. FTP/HTTP сервер продолжает использовать старый интерфейс → ПК не может подключиться. Помогает только переключение на другую Wi-Fi сеть.

**Решение:** кнопка "Обновить сеть" в экране передачи:
1. `wifiManager.disconnect()` + `wifiManager.reconnect()` — принудительный переконнект
2. Пересканировать сетевые интерфейсы, обновить IP в UI
3. Перезапустить HTTP/FTP сервер если запущен
4. Показать новый IP пользователю
...
- Файлы: `I:\...\Haron\4PDA\` (4pda_features_all.md, 4pda_tc_deep.md, 4pda_mixplorer_deep.md, 4pda_solid_deep.md, 4pda_xplore_es_deep.md, 4pda_research_batch1/2.md, 4pda_raw/, html-страницы)
...
**ext4 USB — реализовано:**
- [x] Thumbnails для картинок (кеш 512 КБ → BitmapFactory)
- [x] Drag-and-drop (через executeCopy/executeMove)
- [x] Архивы — создание из ext4 файлов (кеш → архив)
- [x] Архивы — распаковка на ext4 (кеш → копия на ext4)
- [x] Теги (работают по ext4:// ключу)
- [x] Корзина — удаление навсегда (нет корзины на removable)
- [x] Batch rename
- [x] Свойства файла (размер, дата, mime, ext4 mount info)
- [x] MIUI: singleTask launchMode (нет перезапуска при USB)
- [x] SD-карта: фильтрация /dev/null и "null" из suppressedPaths
- [x] Кеш-менеджер: 200 МБ лимит, автоочистка, 50 МБ для не-медиа
- [x] Большие медиа: копирование с прогрессом → воспроизведение
- [x] Большие не-медиа: тост "скопируйте на устройство"
- [x] Тосты: "ext4 обнаружен", "подключён"

**ext4 USB — дополнительно реализовано (сессия 29.03.2026):**
- [x] Chunked SCSI read/write (64 сектора max) — убраны buffer overflow и USB timeout
- [x] Chunked file copy (1 МБ) — убран OOM crash на больших файлах
- [x] nativeWriteFileMode ("wb"/"ab") — append mode для чанкового копирования
- [x] ext4_cache_flush после каждой write-операции — данные persist-ятся
- [x] mtime устанавливается при записи (было 01.01.1970)
- [x] ext4_dir_rm для директорий (ext4_fremove не удаляла папки)
- [x] Drawer: скрывает "unsupported FS" когда ext4 смонтирован
- [x] Progress bar для copy/move с именем файла и процентом
- [x] Live refresh — файлы появляются по одному при копировании
- [x] Голос "обновить" — обе панели сразу
...
### Batch 108 — ext4 USB без root ✅ проверено

Первый Android файловый менеджер с чтением/записью ext4 USB без root.
Архитектура: libaums (raw USB block I/O) → JNI bridge → lwext4 (C) → Kotlin API.
...
- `CMakeLists.txt` — lwext4 static lib + haron_ext4 shared lib через NDK
- `IBlockDevice.kt` — интерфейс raw block I/O
- `LibaumsBlockDevice.kt` — адаптер libaums → IBlockDevice
- `Ext4Native.kt` — JNI объявления
- `Ext4UsbManager.kt` — высокоуровневый API

**Реализовано:**
- Все файловые операции: copy, move, delete, rename, mkdir
- Chunked SCSI I/O (64 сектора) + chunked file copy (1 МБ) — без OOM и USB timeout
- nativeWriteFileMode ("wb"/"ab") — append mode для чанкового копирования
- ext4_cache_flush после каждой write-операции
- mtime устанавливается при записи
- ext4_dir_rm для директорий
- Lazy thumbnails с IoScheduler + disk cache (0–500 МБ в Settings)
- Превью фото — полноразмерное через IoScheduler
- DnD, conflict dialog, executeCopyWithDecisions, executeMoveWithDecisions
- Архивы — создание из ext4 файлов и распаковка на ext4
- Теги (работают по ext4:// ключу)
- Batch rename, свойства файла
- Drawer: скрывает "unsupported FS" когда ext4 смонтирован
- Progress bar для copy/move с именем файла и процентом
- Live refresh — файлы появляются по одному при копировании
- singleTask launchMode — нет перезапуска при USB attach (MIUI)
- suppressedPaths: фильтрация /dev/null и "null"
- ext4, ext3, ext2 filesystems supported
- Анимация скрыта в gallery/player/reader
...
### Batch 102 — Аудит кода: логирование, KDoc, архитектура ✅ проверено
...
### Batch 101 — Свайп-назад, прогресс-бар, аудиоплеер фиксы ✅ проверено

**Свайп от левого края = назад (все экраны):**
- Утилита `SwipeBackModifier.kt` — edge swipe (зона 40dp, порог 80dp)
- Добавлен в: Настройки, Жесты/Голос, Навбар настройки, Навбар иконки, Логи, Анимации (ThemesScreen), Matrix, Snowfall, AnimSettings, Терминал, Библиотека, Анализатор памяти, Дубликатор, Управление метками, Назначение меток, Аудиоплеер

**Прогресс-бар — поднят над кастомным навбаром:**
- Было: 8dp от низа → скрыт за навбаром (48dp)
- Стало: 56dp (без selection), 112dp (с selection bar)

**TagAssignDialog → полноэкранный экран:**
- Был AlertDialog → стал fullscreen Dialog + Scaffold + свайп-назад

**TagManageDialog → раздельные экраны:**
- Основной: "Управление метками" (список, стрелка назад)
- "+" → "Создание метки" (отдельный экран, стрелка назад, свайп назад)
- Редактирование → тот же экран с заголовком "Редактировать метку"
- Навигация: TagManage ↔ TagAssign (возврат из управления в назначение если открыли оттуда)

**Аудиоплеер — 3 фикса:**
- `setPlaylist` для аудио: раньше НЕ вызывался при открытии через тап на файл → играл предыдущий трек. Исправлено.
- Gesture overlay не перехватывает тапы по кнопкам: для аудио обрабатывает только edge-swipe, остальное идёт к controls
- Автоплей отключён: аудио загружается но не играет автоматически, нужно нажать play
- Аудио всегда с начала: позиция не восстанавливается (убрано в VlcPlayerAdapter + MediaPlayerScreen)
...
### Batch 98 — Termux-стиль терминал, SSH через JSch, VT100 эмуляция ✅ проверено
...
### Batch 96 — Терминал: pinch-zoom, SSH resize, copy/paste, полная изоляция табов ✅ проверено
...
### Batch 93 — Видеоплеер: свайпы, настройки DND, описание функций ✅ проверено
...
- Web Navigator: кнопка "Обзор" у найденных директорий → просматривай ссылки на странице, навигируй вглубь, тап на файл = скачивание ⏳ в ожидании
