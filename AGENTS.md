...
**Текущая версия:** 0.12.0 (Phase 1+3, Batch 12)
**Текущая фаза:** Phase 1 — MVP (SAF UI), Phase 3 — архивы + галерея + PDF
...
Batch 12 завершён. Внутренняя галерея (fullscreen + zoom + pager), PDF-читалка (multi-page + zoom + go-to), работа с архивами (browse ZIP/7z/RAR + extract + create ZIP), кнопки "Открыть в галерее/читалке/архиве" из Quick Preview.
...
- Разделитель панелей: перетаскивание для изменения соотношения, двойной тап → 50/50
...
- Избранные папки (добавление/удаление)
- Недавние папки (последние 10)
- Панель избранного через ModalBottomSheet
...
- Кнопка "Открыть в читалке" для PDF
