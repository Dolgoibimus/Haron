...
**Матрица-анимация (следующая задача):**
- ⬜ MatrixRainCanvas — composable с Canvas, катакана/цифры/латиница/кириллица, 30fps
- ⬜ Режим 1: символы за контентом (видны в промежутках)
- ⬜ Режим 2: символы везде, UI поверх с зелёным glow по контуру
- ⬜ MatrixSettingsScreen (кнопка "Темы" в настройках):
  - Вкл/выкл, режим 1/2, палитра цвета, скорость, плотность, прозрачность, набор символов
  - Опция "только при зарядке"
- ⬜ Работает: ExplorerScreen, настройки, библиотека, терминал, галерея, аудиоплеер, все утилиты
- ⬜ НЕ работает: читалки (Doc/PDF), видеоплеер
- ⬜ Автопауза при onPause (батарея)
