...
- [ ] Поиск FTS5 по ext4 (нужна отдельная индексация)
- [ ] Стриминг/каст (кеш-based, сделать когда понадобится)
- [ ] Подавление системного "нет доступа" уведомления
- [ ] Проценты по чанкам при копировании + общий размер файлов
- [ ] Превью/thumbnails (отключены из-за USB single-thread)
