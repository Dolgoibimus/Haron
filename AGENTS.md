...
Cast-фиксы: PartialContent для Chromecast, громкость 0.01 + физ.кнопки, кнопка отключения в оверлее, DLNA-логирование.
Bluetooth: переведён на стандартный OPP (работает с любым устройством), RFCOMM-хэндшейк исправлен, BT-приём интегрирован в ReceiveFileManager. **Bluetooth не дотестирован — продолжить завтра.**

### План до релиза 1.0:
| Батч | Задача | Статус |
|------|--------|--------|
| 30 | Поиск в архивах + OCR + USB OTG | ✅ проверено |
| 31 | Сетевое обнаружение | ⚠️ не проверено |
...
| 51 | SMB-браузер (вкладка в Передача) | ⚠️ не проверено |
| 51b | Двухпанельный SMB-режим (SMB + локальные файлы) | ⚠️ не проверено |
