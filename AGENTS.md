...
- ⬜ ARROW_UP/ARROW_DOWN — курсор по файлам, long = переключение панели
- ⬜ Долгий тап с кружком — продумать на какие кнопки и как (отложено)

**Следующие задачи навбара (после radial menu):**
...
**4. Radial menu для кнопок навбара** ⚠️ не проверено
...
- Палец ведёшь к кружку → подсвечивается, отпускаешь → действие, мимо → отмена
- Copy/Move: левый кружок = Copy, правый = Move
- Delete: левый = Корзина, правый = Навсегда
- Create: левый = Файл, правый = Папка
