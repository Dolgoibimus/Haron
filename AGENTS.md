...
**Матрица-анимация** ✅ реализовано:
- ✅ MatrixRainCanvas — Canvas с падающими символами, ~30fps через awaitFrame + tick state
- ✅ Один режим: символы поверх всего контента с glow на головах столбцов
- ✅ MatrixSettingsScreen (Настройки → Matrix Rain):
  - Вкл/выкл, палитра цвета (8 пресетов + RGB слайдеры), скорость, плотность, прозрачность
  - Набор символов (катакана/бинарный/латиница/кириллица/микс)
  - Опция "только при зарядке" (BatteryManager.isCharging)
  - Превью анимации вверху экрана
- ✅ Работает везде кроме читалок (Doc/PDF) и видеоплеера
- ✅ Навбар прозрачный при включённой матрице
- ✅ Реактивный конфиг через SharedPreferences.OnSharedPreferenceChangeListener
- ✅ Автопауза при onPause (LifecycleEventObserver)
- ✅ Колонки пересоздаются при смене speed/density/charset
