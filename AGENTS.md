...
**Текущая версия:** 0.17.1 (Phase 1+3, Post-Batch 17)
...
Post-Batch 17 доработки завершены. Удалён лог операций (заменён на принудительное удаление). Пофайловый прогресс для всех операций. Пай-меню закладок (полукруг). 3 зоны на разделителе панелей. Лимит корзины в настройках.
...
- Секция "Инструменты": корзина, анализатор памяти, детектор дубликатов, пустые папки, принудительное удаление
...
### Закладки быстрого доступа (1-9)
- Тап на левую зону разделителя панелей → пай-меню (полукруг из 9 кружков)
- Пустой слот: номер + приглушённый вид
- Сохранённый слот: номер + имя папки
- Короткий тап на сохранённый слот → мгновенная навигация
- Долгий тап на слот → сохранить текущую папку
- Закладки сохраняются между запусками

### Разделитель панелей
- Перетаскивание для изменения соотношения панелей
- 3 тап-зоны: левая (закладки), центральная (сброс 50/50), правая (пока не определена)
