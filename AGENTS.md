...
Релиз 1.0 готов. Все батчи проверены. Стеганография скрыта до отдельного релиза.
Последние изменения: анализатор хранилища — свои топ-200 файлов на категорию (без порога 10 МБ),
тап-зоны в сетке (иконка: тап→превью/долгий→выделение, имя: тап→открыть/долгий→DnD).
У пользователя есть вопросы по SMB (батч 51/51b).
...
| Сетевой поток на Cast | Ввод URL (HTTP/HLS/DASH) и трансляция на Chromecast/DLNA без скачивания |
...
- Активная панель подсвечена рамкой
- Запоминание позиции скрола в каждой панели — при возврате с другого экрана список не сбрасывается в начало

### Файлы и папки
- Список файлов с иконками по типу
- Эскизы (превью) прямо в иконках: изображения, видео, текст/код, PDF, документы, APK, FB2/FB2.ZIP
- Масштабируемая сетка: от 1 до 6 колонок через pinch-жест, иконки адаптивно заполняют ячейку
- Тап-зоны в сетке: тап на иконку → превью, долгий тап на иконку → выделение, тап на имя → открыть, долгий тап на имя → быстрая отправка
...
- Выбрать все / снять все (долгий тап — меню выбора по расширению)
...
- FB2/FB2.ZIP: обложка книги слева с обтеканием текстом аннотации и начала книги
...
- Кнопка микрофона автоматически скрывается при воспроизведении видео, появляется при паузе
...
- Кнопка микрофона скрывается при просмотре, появляется по тапу вместе с контролами
...
- Кнопка микрофона скрывается при чтении, появляется по тапу на 3 секунды
...
- Запоминание страницы при закрытии — при повторном открытии PDF продолжается с того же места
- Запароленные PDF: при открытии запрашивается пароль, расшифровка и отображение
- Зашифрованные документы (DOC/DOCX): показ сообщения "Зашифрованные документы не поддерживаются"

### Читалка документов
- Кнопка микрофона скрывается при чтении, появляется по тапу на 3 секунды
- Чтение DOC, DOCX, ODT, RTF, FB2, XLSX, XLS, CSV файлов прямо в приложении
- Открытие файлов .fb2.zip как книг FB2 (автоматическая распаковка из ZIP)
- FB2: обложка книги и аннотация/описание отображаются перед основным текстом
...
- Запоминание позиции прокрутки при закрытии — при повторном открытии продолжается с того же места
- Таблицы (XLSX, XLS, CSV): пропорциональные ширины колонок, рамки по стилю ячейки, заголовки вне таблицы
- Безрамочные таблицы в DOCX/ODT (шапки «УТВЕРЖДАЮ» и т.п.) отображаются без сетки

### Работа с архивами
- Просмотр содержимого ZIP, 7z, RAR (включая RAR5) архивов
- **Inline-просмотр**: архив открывается прямо в панели как обычная папка (тап на ZIP/7z/RAR)
- Навигация по папкам внутри архива, breadcrumbs виртуального пути
- Кнопка «Назад» — наверх по уровням, из корня архива — обратно в обычную папку
- Режим выделения для выборочного извлечения
- Извлечение в **активную панель** (с рамкой): если рамка на той же панели — рядом с архивом, если на другой — в папку другой панели
- Прогресс извлечения в реальном времени
- Прогресс создания архива пофайлово в реальном времени
- Создание ZIP из выделенных файлов (кнопка ZIP в панели действий)
- Диалог ввода имени архива
- Создание ZIP с паролем (AES-256 шифрование)
- Создание ZIP с разбиением на части (split ZIP: .zip, .z01, .z02...)
- Пароль + разбиение на части — вместе
- Предустановки размера частей: 100, 500, 700, 1024, 2048 МБ или свой
- Запароленные архивы: при открытии запрашивается пароль, при ошибке — повторный ввод с сообщением "Неверный пароль"
- Поддержка паролей для ZIP, 7z, RAR

### Текстовый редактор
- Просмотр и редактирование TXT и MD файлов прямо в приложении
- По умолчанию открывается в режиме просмотра (текст нельзя менять, микрофон виден)
- Кнопка "Редактировать" — переход в режим редактирования (микрофон скрывается)
- Кнопка "Сохранить" — сохранение и возврат в режим просмотра (микрофон возвращается)
- Моноширинный шрифт
- Undo/Redo (до 50 шагов) в режиме редактирования
- Сохранение файла (включая SAF content:// URI)
- Диалог "Сохранить / Не сохранять / Остаться" при выходе с несохранёнными изменениями
- Лимит 1 МБ с предупреждением
- Индикатор изменений и позиция курсора в статус-баре
- Запоминание позиции курсора при закрытии — при повторном открытии курсор на прежнем месте
...
- Файлы без встроенного обработчика (презентации и др.) автоматически открываются во внешнем приложении
...
- Тап на категорию → топ-200 крупнейших файлов этой категории (первые 10 + кнопка "Показать все")
- Секция "Крупные файлы (>10 МБ)" — топ-200 по размеру (первые 10 + кнопка "Показать все")
...
- Кеш размеров папок (повторное выделение — мгновенно, обновляется после операций с файлами)
...
- Поиск по содержимому индексирует текст, архивы, картинки (EXIF + OCR) в текущей папке
- В корне (где только папки без файлов) — автоматически индексирует файлы из ближайших подпапок
- Повторное индексирование использует кеш — уже проиндексированные файлы пропускаются
...
### Cast на ТВ
- Кнопка Cast в медиаплеере и галерее (видна всегда — DLNA работает без GMS)
- Поиск Chromecast, Miracast и DLNA устройств одним диалогом
- Google Cast (Chromecast): трансляция видео, аудио и фото на ТВ
- Miracast: зеркалирование экрана телефона на ТВ
- DLNA/UPnP: трансляция на Smart TV без Chromecast (SSDP discovery + SOAP control)
- HTTP-сервер для раздачи медиафайлов на Chromecast и DLNA-рендереры
- Пульт управления: появляется автоматически при подключении к Chromecast или DLNA
- Play/Pause, перемотка (seekbar), громкость +/− (для Chromecast и DLNA)
...
- Тап на SMB-шару → подключение и просмотр содержимого (SMB-браузер)

### SMB-браузер
- Вкладка "SMB" на экране "Передача файлов" — полноценный браузер SMB-шар в локальной сети
- Список обнаруженных SMB-серверов (из автоматического сканирования сети)
- Список сохранённых серверов с возможностью удаления
- Подключение вручную по IP-адресу
- Авторизация: логин/пароль + домен (опционально), вход как гость
- Сохранение данных входа (зашифрованное хранилище AES-256-GCM)
- Просмотр общих папок (шар) на сервере
- Навигация по папкам внутри шары как в обычном проводнике
- Хлебные крошки с кликабельными сегментами пути (хост > шара > папка)
- Скачивание файлов на устройство с прогрессом
- Загрузка файлов с устройства на SMB-шару с прогрессом
- Создание папок на SMB
- Удаление файлов и папок на SMB
- Переименование файлов и папок на SMB
- Мульти-выбор файлов (долгий тап) с массовым скачиванием/удалением
- Автоподключение при сохранённых данных входа
- Фильтрация системных шар (IPC$, ADMIN$, C$)
- Конфликт при скачивании — автоматическое добавление суффикса (1), (2)
- **Двухпанельный режим** после подключения: верхняя панель — SMB-файлы, нижняя — файлы устройства
- Перетаскиваемый разделитель между панелями (как в основном проводнике)
- Тап по центру разделителя — сброс размера 50/50
- Скачивание SMB-файлов прямо в текущую папку нижней (локальной) панели
- Загрузка файлов с устройства на SMB прямо из нижней панели (без пикера)
- Создание/удаление/переименование папок и файлов в обеих панелях
- Индикация активной панели (цветной бордер)
- Навигация кнопкой «Назад»: сначала по активной панели, потом переключение панели, потом отключение
...
- Кнопка «Сравнить» в панели действий при выделении 2 файлов/папок в одной панели или по 1 в каждой панели
