...
// POI not available in mini — .doc preview disabled
...
class LoadPreviewUseCase @Inject constructor(
        ...
        try {
            ...
            val effectiveEntry = if (entry.path.startsWith("ext4://")) {
                ...
                if (cacheFile.exists() && cacheFile.length() > 0 &&
                    ...
                    if (data != null) { cacheFile.writeBytes(data); entry.copy(path = cacheFile.absolutePath) } else entry
                }
            } else entry
            ...
            val result = when {
                ...
                else -> PreviewData.UnsupportedPreview(fileName = e.name, fileSize = e.size, lastModified = e.lastModified, mimeType = e.extension)
            }
    ...
    private fun loadImage(entry: FileEntry): PreviewData {
        ...
        if (entry.isContentUri) context.contentResolver.openInputStream(Uri.parse(entry.path))?.use { BitmapFactory.decodeStream(it, null, options) } else BitmapFactory.decodeFile(entry.path, options)
        val rawWidth = options.outWidth; val rawHeight = options.outHeight
        val orientation = try { val exif = if (entry.isContentUri) context.contentResolver.openInputStream(Uri.parse(entry.path))?.use { ExifInterface(it) } else ExifInterface(entry.path); exif?.getAttributeInt(ExifInterface.TAG_ORIENTATION, ExifInterface.ORIENTATION_NORMAL) ?: ExifInterface.ORIENTATION_NORMAL } catch (_: Exception) { ExifInterface.ORIENTATION_NORMAL }
        val sampleSize = calculateInSampleSize(rawWidth, rawHeight, MAX_IMAGE_SIZE, MAX_IMAGE_SIZE)
        var bitmap = (if (entry.isContentUri) context.contentResolver.openInputStream(Uri.parse(entry.path))?.use { BitmapFactory.decodeStream(it, null, BitmapFactory.Options().apply { inSampleSize = sampleSize }) } else BitmapFactory.decodeFile(entry.path, BitmapFactory.Options().apply { inSampleSize = sampleSize })) ?: throw IllegalStateException("Cannot decode image")
        val matrix = Matrix()
        when (orientation) { ExifInterface.ORIENTATION_ROTATE_90 -> matrix.postRotate(90f); ExifInterface.ORIENTATION_ROTATE_180 -> matrix.postRotate(180f); ExifInterface.ORIENTATION_ROTATE_270 -> matrix.postRotate(270f); ExifInterface.ORIENTATION_FLIP_HORIZONTAL -> matrix.preScale(-1f, 1f); ExifInterface.ORIENTATION_FLIP_VERTICAL -> matrix.preScale(1f, -1f); ExifInterface.ORIENTATION_TRANSPOSE -> { matrix.postRotate(90f); matrix.preScale(-1f, 1f) }; ExifInterface.ORIENTATION_TRANSVERSE -> { matrix.postRotate(270f); matrix.preScale(-1f, 1f) } }
    ...
    private fun loadVideo(entry: FileEntry): PreviewData { val retriever = MediaMetadataRetriever(); try { try { if (entry.isContentUri) retriever.setDataSource(context, Uri.parse(entry.path)) else retriever.setDataSource(entry.path) } catch (_: Exception) { return PreviewData.VideoPreview(fileName = entry.name, fileSize = entry.size, lastModified = entry.lastModified, thumbnail = null, durationMs = 0L) }; val durationMs = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_DURATION)?.toLongOrNull() ?: 0L; val thumbnail = try { retriever.getFrameAtTime(0, MediaMetadataRetriever.OPTION_CLOSEST_SYNC) } catch (_: Exception) { null }; return PreviewData.VideoPreview(fileName = entry.name, fileSize = entry.size, lastModified = entry.lastModified, thumbnail = thumbnail, durationMs = durationMs) } finally { retriever.release() } }

    private fun loadAudio(entry: FileEntry): PreviewData { val retriever = MediaMetadataRetriever(); try { if (entry.isContentUri) retriever.setDataSource(context, Uri.parse(entry.path)) else retriever.setDataSource(entry.path); val durationMs = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_DURATION)?.toLongOrNull() ?: 0L; val albumArt = retriever.embeddedPicture?.let { BitmapFactory.decodeByteArray(it, 0, it.size) }; return PreviewData.AudioPreview(fileName = entry.name, fileSize = entry.size, lastModified = entry.lastModified, albumArt = albumArt, title = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_TITLE), artist = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_ARTIST), album = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_ALBUM), durationMs = durationMs) } finally { retriever.release() } }

    private fun loadText(entry: FileEntry): PreviewData { val lines = mutableListOf<String>(); var totalLines = 0; val reader = if (entry.isContentUri) BufferedReader(InputStreamReader(context.contentResolver.openInputStream(Uri.parse(entry.path)) ?: throw IllegalStateException("Cannot open text file"))) else File(entry.path).bufferedReader(); reader.use { br -> var line = br.readLine(); while (line != null) { totalLines++; if (lines.size < MAX_TEXT_LINES) lines.add(line); line = br.readLine() } }; return PreviewData.TextPreview(fileName = entry.name, fileSize = entry.size, lastModified = entry.lastModified, content = lines.joinToString("\n"), totalLines = totalLines, extension = entry.extension) }

    private fun loadPdf(entry: FileEntry): PreviewData { val pfd = if (entry.isContentUri) context.contentResolver.openFileDescriptor(Uri.parse(entry.path), "r") ?: throw IllegalStateException("Cannot open PDF") else ParcelFileDescriptor.open(File(entry.path), ParcelFileDescriptor.MODE_READ_ONLY); pfd.use { descriptor -> val renderer = PdfRenderer(descriptor); renderer.use { pdf -> val pageCount = pdf.pageCount; val page = pdf.openPage(0); val scale = MAX_IMAGE_SIZE.toFloat() / maxOf(page.width, page.height).coerceAtLeast(1); val bmpWidth = (page.width * scale).toInt().coerceAtLeast(1); val bmpHeight = (page.height * scale).toInt().coerceAtLeast(1); val bitmap = Bitmap.createBitmap(bmpWidth, bmpHeight, Bitmap.Config.ARGB_8888); bitmap.eraseColor(android.graphics.Color.WHITE); page.render(bitmap, null, null, PdfRenderer.Page.RENDER_MODE_FOR_DISPLAY); page.close(); return PreviewData.PdfPreview(fileName = entry.name, fileSize = entry.size, lastModified = entry.lastModified, firstPage = bitmap, pageCount = pageCount, filePath = entry.path) } } }

    private fun loadArchive(entry: FileEntry): PreviewData { val type = BrowseArchiveUseCase.archiveType(entry.path); return when (type) { "zip" -> loadZip(entry); "7z" -> load7z(entry); "rar" -> loadRar(entry); "tar", "tar.gz", "tar.bz2", "tar.xz" -> loadTar(entry, type); else -> PreviewData.UnsupportedPreview(fileName = entry.name, fileSize = entry.size, lastModified = entry.lastModified, mimeType = entry.extension) } }

    private fun loadZip(entry: FileEntry): PreviewData { val file = if (entry.isContentUri) copyToTemp(entry) else File(entry.path); try { val zipFile = java.util.zip.ZipFile(file); return zipFile.use { zf -> val entries = zf.entries().asSequence().toList(); if (entries.isEmpty()) throw IllegalStateException(context.getString(R.string.archive_password_or_corrupt)); PreviewData.ArchivePreview(fileName = entry.name, fileSize = entry.size, lastModified = entry.lastModified, entries = entries.take(MAX_ARCHIVE_ENTRIES).map { e -> ArchiveEntryInfo(e.name, e.size.coerceAtLeast(0), e.isDirectory) }, totalEntries = entries.size, totalUncompressedSize = entries.sumOf { it.size.coerceAtLeast(0) }) } } catch (e: java.util.zip.ZipException) { throw IllegalStateException(context.getString(R.string.archive_password_or_corrupt)) } finally { if (entry.isContentUri) file.delete() } }

    private fun load7z(entry: FileEntry): PreviewData { val file = if (entry.isContentUri) copyToTemp(entry) else File(entry.path); try { val channel = if (entry.isContentUri) SeekableInMemoryByteChannel(file.readBytes()) else java.nio.channels.FileChannel.open(file.toPath(), java.nio.file.StandardOpenOption.READ); channel.use { ch -> val sevenZ = SevenZFile.builder().setSeekableByteChannel(ch).get(); sevenZ.use { archive -> val allEntries = mutableListOf<ArchiveEntryInfo>(); var totalSize = 0L; for (e in archive.entries) { totalSize += e.size.coerceAtLeast(0); if (allEntries.size < MAX_ARCHIVE_ENTRIES) allEntries.add(ArchiveEntryInfo(e.name, e.size.coerceAtLeast(0), e.isDirectory)) }; return PreviewData.ArchivePreview(fileName = entry.name, fileSize = entry.size, lastModified = entry.lastModified, entries = allEntries, totalEntries = allEntries.size, totalUncompressedSize = totalSize) } } } finally { if (entry.isContentUri) file.delete() } }

    private fun loadRar(entry: FileEntry): PreviewData { val file = if (entry.isContentUri) copyToTemp(entry) else File(entry.path); try { return try { val archive = Archive(file); archive.use { rar -> val allEntries = mutableListOf<ArchiveEntryInfo>(); var totalSize = 0L; for (h in rar.fileHeaders ?: emptyList()) { val size = h.fullUnpackSize.coerceAtLeast(0); totalSize += size; if (allEntries.size < MAX_ARCHIVE_ENTRIES) allEntries.add(ArchiveEntryInfo(name = h.fileName ?: "?", size = size, isDirectory = h.isDirectory)) }; PreviewData.ArchivePreview(fileName = entry.name, fileSize = entry.size, lastModified = entry.lastModified, entries = allEntries, totalEntries = allEntries.size, totalUncompressedSize = totalSize) } } catch (e: Throwable) { EcosystemLogger.d(HaronConstants.TAG, "loadRar: junrar failed (${e.javaClass.simpleName}), trying 7-Zip-JBinding"); loadRarWith7Zip(file, entry) } } finally { if (entry.isContentUri) file.delete() } }

    private fun loadRarWith7Zip(file: File, entry: FileEntry): PreviewData { net.sf.sevenzipjbinding.SevenZip.initSevenZipFromPlatformJAR(); val raf = java.io.RandomAccessFile(file, "r"); val stream = net.sf.sevenzipjbinding.impl.RandomAccessFileInStream(raf); val archive = net.sf.sevenzipjbinding.SevenZip.openInArchive(null, stream); try { val count = archive.numberOfItems; val allEntries = mutableListOf<ArchiveEntryInfo>(); var totalSize = 0L; for (i in 0 until count) { val path = (archive.getProperty(i, net.sf.sevenzipjbinding.PropID.PATH) as? String ?: "").replace('\\', '/'); val isDir = archive.getProperty(i, net.sf.sevenzipjbinding.PropID.IS_FOLDER) as? Boolean ?: false; val size = (archive.getProperty(i, net.sf.sevenzipjbinding.PropID.SIZE) as? Long) ?: 0L; totalSize += size; if (allEntries.size < MAX_ARCHIVE_ENTRIES) allEntries.add(ArchiveEntryInfo(name = path.trimEnd('/').substringAfterLast('/'), size = size, isDirectory = isDir)) }; return PreviewData.ArchivePreview(fileName = entry.name, fileSize = entry.size, lastModified = entry.lastModified, entries = allEntries, totalEntries = allEntries.size, totalUncompressedSize = totalSize) } finally { archive.close(); stream.close(); raf.close() } }

    private fun loadTar(entry: FileEntry, type: String): PreviewData { val file = if (entry.isContentUri) copyToTemp(entry) else File(entry.path); try { val rawStream = java.io.BufferedInputStream(java.io.FileInputStream(file)); val decompressedStream: java.io.InputStream = when (type) { "tar.gz" -> org.apache.commons.compress.compressors.gzip.GzipCompressorInputStream(rawStream); "tar.bz2" -> org.apache.commons.compress.compressors.bzip2.BZip2CompressorInputStream(rawStream); "tar.xz" -> org.apache.commons.compress.compressors.xz.XZCompressorInputStream(rawStream); else -> rawStream }; org.apache.commons.compress.archivers.tar.TarArchiveInputStream(decompressedStream).use { tar -> val allEntries = mutableListOf<ArchiveEntryInfo>(); var totalSize = 0L; var e = tar.nextEntry; while (e != null) { totalSize += e.size.coerceAtLeast(0); if (allEntries.size < MAX_ARCHIVE_ENTRIES) allEntries.add(ArchiveEntryInfo(e.name.trimEnd('/').substringAfterLast('/'), e.size.coerceAtLeast(0), e.isDirectory)); e = tar.nextEntry }; return PreviewData.ArchivePreview(fileName = entry.name, fileSize = entry.size, lastModified = entry.lastModified, entries = allEntries, totalEntries = allEntries.size, totalUncompressedSize = totalSize) } } finally { if (entry.isContentUri) file.delete() } }

    @Suppress("DEPRECATION")
    private fun loadApk(entry: FileEntry): PreviewData { val file = if (entry.isContentUri) copyToTemp(entry) else File(entry.path); try { val pm = context.packageManager; val info = pm.getPackageArchiveInfo(file.absolutePath, 0); if (info != null) { info.applicationInfo?.sourceDir = file.absolutePath; info.applicationInfo?.publicSourceDir = file.absolutePath; val appName = info.applicationInfo?.loadLabel(pm)?.toString(); val icon = try { info.applicationInfo?.loadIcon(pm)?.let { drawable -> val bmp = Bitmap.createBitmap(drawable.intrinsicWidth.coerceAtLeast(1), drawable.intrinsicHeight.coerceAtLeast(1), Bitmap.Config.ARGB_8888); val canvas = android.graphics.Canvas(bmp); drawable.setBounds(0, 0, canvas.width, canvas.height); drawable.draw(canvas); bmp } } catch (_: Exception) { null }; return PreviewData.ApkPreview(fileName = entry.name, fileSize = entry.size, lastModified = entry.lastModified, appName = appName, packageName = info.packageName, versionName = info.versionName, versionCode = info.longVersionCode, icon = icon) }; return PreviewData.ApkPreview(fileName = entry.name, fileSize = entry.size, lastModified = entry.lastModified, appName = null, packageName = null, versionName = null, versionCode = 0, icon = null) } finally { if (entry.isContentUri) file.delete() } }
    ...
    private fun loadDoc(entry: FileEntry): PreviewData = PreviewData.UnsupportedPreview(fileName = entry.name, fileSize = entry.size, lastModified = entry.lastModified, mimeType = "doc")
