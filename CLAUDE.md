...
После каждого изменения кода — запускать сборку самостоятельно:
```bash
cd /c/Users/User/AndroidStudioProjects/Haron && bash gradlew assembleDebug 2>&1 | tail -20
```
Если падает — чинить и пересобирать, не спрашивая пользователя.
